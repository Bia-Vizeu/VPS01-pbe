...
const update = (req, res) => {
    ...
    let query = `UPDATE carros SET marca_veiculo = '${marca_veiculo}', modelo_veiculo = '${modelo_veiculo}',ano_veiculo ='${ano_veiculo}', fabricacao_veiuclo = '${fabricacao_veiuclo}', cliente_id = '${cliente_id}' `
    con.query(query, (err, result) => {
        if (err) {
            res.status(500).json(err)
        } else {
